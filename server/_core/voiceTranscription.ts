...
export async function transcribeAudio(
  ...
  try {
    ...
    const transcriptionTimeout = setTimeout(() => transcriptionController.abort(), 120000); // 120s timeout
    ...
    try {
      ...
      const whisperResponse = await response.json().catch(() => {
        return {
          error: "Invalid JSON response from transcription service",
          code: "SERVICE_ERROR" as const,
          details: "Failed to parse transcription response",
        };
      }) as WhisperResponse | TranscriptionError;

      if ("error" in whisperResponse) {
      ...
      if (fetchError instanceof Error && fetchError.name === "AbortError") {
        return {
          ...
          code: "SERVICE_ERROR",
          details: "Request exceeded 120 second timeout",
        };
      }
      throw fetchError;
    }

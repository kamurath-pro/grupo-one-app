...
class OAuthService {
  ...
  private decodeState(state: string): string {
    ...
    try {
      ...
      if (!redirectUri || redirectUri.length === 0) {
        throw new Error("Decoded state is empty");
      }
      return redirectUri;
    } catch (error) {
  ...
  async getTokenByCode(code: string, state: string): Promise<ExchangeTokenResponse> {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(`OAuth token exchange failed: ${error.response?.status} ${error.response?.statusText || error.message}`);
      }
      throw error;
    }
  }
  ...
  async getUserInfoByToken(token: ExchangeTokenResponse): Promise<GetUserInfoResponse> {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(`OAuth user info request failed: ${error.response?.status} ${error.response?.statusText || error.message}`);
      }
      throw error;
    }
  }
...
class SDKServer {
  ...
  async getUserInfoWithJwt(jwtToken: string): Promise<GetUserInfoWithJwtResponse> {
    ...
    try {
      ...
      if (axios.isAxiosError(error)) {
        throw new Error(`OAuth JWT user info request failed: ${error.response?.status} ${error.response?.statusText || error.message}`);
      }
      throw error;
    }
  }
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    if (!sessionUserId || typeof sessionUserId !== "string" || sessionUserId.length === 0) {
    ...
    if (!user) {
      try {
        if (!sessionCookie) {
          throw new Error("Session cookie is required for user sync");
        }
        const userInfo = await this.getUserInfoWithJwt(sessionCookie);
        
        if (!userInfo.openId) {
          throw new Error("OAuth response missing openId");
        }
    ...
    try {
      ...
      // Log but don't fail authentication if lastSignedIn update fails
